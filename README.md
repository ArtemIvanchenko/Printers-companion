...
**Иконка на рабочий стол (один раз):**
```powershell
powershell -ExecutionPolicy Bypass -File deploy\create-desktop-shortcut.ps1
```
Появится ярлык **Printer's Companion**. Дальше запускать систему —
просто дважды кликнуть по нему: он сам проверит обновления на GitHub,
скачает новую версию если есть, запустит контейнеры и откроет дашборд
в браузере. Этим же способом обновляется — отдельно ничего делать не нужно.

Для автозапуска при включении ПК положи ярлык `deploy\autostart-windows.bat`
в `shell:startup` — при загрузке система тоже проверит обновления сама.
