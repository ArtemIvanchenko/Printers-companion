...
Карта модулей и правила размещения нового кода: [docs/PROJECT_STRUCTURE.md](docs/PROJECT_STRUCTURE.md).
Схема общего NAS и операторских ПК: [deploy/nas/README.md](deploy/nas/README.md).
Методы аналитики и ворота допуска моделей: [docs/ADVANCED_ANALYTICS.md](docs/ADVANCED_ANALYTICS.md).
...
- **Аудит логов** — потоковая проверка структуры, временной шкалы и опциональный MLP-анализ (`pla audit-log`)
...
Обновление происходит **только через иконку на рабочем столе** — при каждом
запуске она сама проверяет GitHub и подтягивает свежую версию, прежде чем
запустить систему. Никаких фоновых служб, задач планировщика или кнопок в
дашборде для этого нет — вкладка «О системе» лишь показывает, есть ли
обновление, но ничего не запускает сама.

- Иконка на рабочем столе (создаётся `deploy\create_shortcut.ps1`) запускает
  `deploy\launch.ps1` — он делает Docker-проверку, `update.ps1` (git pull +
  перезапуск сервисов), затем открывает дашборд.
- Либо `deploy\launchers\Запустить.bat` / `Запустить.command` — самодостаточные
  версии (умеют склонировать проект с нуля при первом запуске), тоже проверяют
  обновления при каждом запуске.

Чтобы обновиться: закройте систему и запустите иконку ещё раз.
