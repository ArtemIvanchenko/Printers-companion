...
1. В `.env` заменить:
   ...
   > **Пароли обязаны быть реальными.** Пока Postgres и MinIO стояли на том же
   > ПК, заглушки `change-me` / `change-me-minio` / `minioadmin` ничего не
   > открывали. После переключения это учётные данные сетевой службы, видимой
   > всей tailnet. Приложение проверяет это на старте: если адрес БД или MinIO
   > не локальный, а пароль остался заглушкой — контейнер откажется
   > стартовать с понятной ошибкой (`core/preflight.py`).
   >
   > Само приложение по-прежнему слушает только `127.0.0.1:8000` на каждом ПК —
   > в сеть выходят именно Postgres и MinIO на NAS, поэтому вся защита здесь
   > держится на этих паролях и на ограничении портов диапазоном Tailscale.
